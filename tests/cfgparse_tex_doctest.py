    ...
    if tag == 'file':
        ...
        for line in body.splitlines():
            if line.strip() == '$':
                ...
                result.append(line)
                p = subprocess.Popen(line[2:], shell=True, stdin=subprocess.PIPE, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
                (fi, fo, fe) = (p.stdin, p.stdout, p.stderr)
                fi.close()
                message = str( fo.read(), encoding='UTF-8')
                if (fe != None): 
                    message += str(fe.read(), encoding='UTF-8')
                ...
                #message =(fo.read()
                #x = str(fo, encoding='UTF-8')
            #    print(x)
            #    fo.close()

            #    p = subprocess.Popen(line[2:], stdout=subprocess.PIPE, stderr=subprocess.STDOUT, shell=True)
            #    (output, err) = p.communicate()
            #    if(id == 'check'):
            #        print(repr(output))

            #    message = str(output, encoding='UTF-8')
            #    if (err != None): 
            #        message += str(err, encoding='UTF-8')
